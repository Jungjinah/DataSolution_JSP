...
function inEmpty(input) {
	return (!input.value);		// 아무것도 안썼을때 return할거임!
}

// <input>, 글자수를 넣었을 때
// 그 글자수보다 더 적으면 true, 그 글자수 이상이면 false
function atLeastLetter(input, len) {
	return (input.vlaue.length < len);		// 내가 입력한 글자수보다 적으면 return할거임!
}

// <input> 넣었을 때
// 한글 / 특수문자가 적혀있다면 true, 없다면 false
function containsAnother(input) {
	let pass = "123456789@abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
...
function isNotType(input, type) {
	// .gif, .png, ...
	type ="." + type;
	return (input.value.indexOf(type) == -1);	// 확장자명이 아니면 return
}
